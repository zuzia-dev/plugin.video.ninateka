...
def getUrl(url,x_origin_url=None):
    if x_origin_url:
...
def home():
    ...
    add_item('https://ninateka.pl/vod/dokument/', 'Dokument', '', True, "listmovies") 
    add_item('https://ninateka.pl/vod/fabula/', 'Fabuła', '', True, "listmovies") 
    add_item('https://ninateka.pl/vod/teatr/', 'Spektakl', '', True, "listmovies") 
    add_item('https://ninateka.pl/vod/ksiazki-czytane/', 'Ksiązki', '', True, "listmovies") 
    add_item('https://ninateka.pl/vod/rozmowy/', 'Rozmowa', '', True, "listmovies") 
    add_item(apiurl+'search?page=1&limit=48&__NodeTypeAlias.0=asset&search_tag.2=tag_1423&sort_field.5=Ascending|https://ninateka.pl/vod/', 'Dla dzieci', '', True, "listsubcateg") 
    add_item(apiurl+'search?page=1&limit=48&__NodeTypeAlias.0=asset&sort_field.5=Ascending|https://ninateka.pl/vod/', 'Wszystkie', '', True, "listsubcateg") 
    add_item('https://ninateka.pl/vod/rozmowy/', 'Szukaj', '', True, "listsearch") 

def ListMovies(xorigin):

    jsondata = getUrl(apiurl+'content',xorigin)
    ...
    for cc in ccs:
        ...
        if cc.get('items',None):
            ...
            urlk = apiurl+'search?page=1&limit=48&__NodeTypeAlias.0=asset&__subCategory.2='+str(id)+'&sort_field.5=Ascending|'+xorigin
            add_item(urlk, tytul, '', True, "listsubcateg") 

    xbmcplugin.endOfDirectory(addon_handle)        

def ListSubcateg(url,pg):

    urlk,xorigin = url.split('|')
    urlk = re.sub('page=\d+\&','page=%d&'%int(pg),urlk)

    jsondata = getUrl(urlk,xorigin)
    ...
    if records:
        ...
        for rc in records:
            ...
            add_item(name=tytul, url=urlk, mode='graj', image=img, folder=False, isplay = True,infoLabels=infoL, itemcount=items)  

        if int(pg)<jsondata.get('pageCount',None):
            add_item(url, 'Nast. strona', '', True, "listsubcateg",page=int(pg)+1) 
